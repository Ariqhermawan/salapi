...
A retained-key Testnet deployment was created on 15 September 2026 and is now
the live D3 contract used by `salapi.app`:
...
available. The three signer rows are read-back/decryption verified with the
existing wallet encryption key; no existing wallet row was replaced. The
initial vault state was paused and empty. The production cutover and
authenticated multi-user live E2E are complete; the current live state is
`Active` with 8.0307692 XLM after the recorded payout run in the
[D3 acceptance report](../instawards/week-3-d3.md).
