    ...
    <div className="px-5">
      ...
      >
        ...
        <button
          ...
          disabled={pending}
          className="mt-5 w-full rounded-xl bg-white py-3 text-sm font-bold text-[var(--color-action-deep)] disabled:opacity-60"
        >
      ...
      {note && (
        <p className="s-anim-up mt-2.5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-relaxed text-[var(--color-warn)]">
          {note}
        </p>
      )}

      <div className="mt-5 grid grid-cols-3 gap-3">
        <Act href="/send" label={t("wallet.send")} d="M22 2 11 13M22 2l-7 20-4-9-9-4 20-7Z" />
        <Act
          href="/vaults"
          label={t("wallet.vaults")}
          d="M4 7V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v2M3 7h18v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7Zm9 5v3"
        />
        <Act
          href="/transparency"
          label={t("wallet.donate")}
...
function Act({ href, label, d }: { href: string; label: string; d: string }) {
  return (
    <Link
      href={href}
